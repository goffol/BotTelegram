...
export const uidSchema = z
  ...
  .regex(
    /^[a-zA-Z0-9_-]+$/,
    "UID may only contain letters, numbers, underscore, and hyphen",
  )
  .refine(
    (v) => !/(--|\/\*|\*\/|;|'|"|<|>|`|\$\(|\b(select|drop|union|insert|script)\b)/i.test(v),
    "UID contains disallowed patterns",
  );
...
export function validateChangeUid(input: unknown): {
  ok: true;
  data: ChangeUidInput;
} | {
  ok: false;
