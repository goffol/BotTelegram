...
function isSuccessPayload(status: number, payload: unknown): boolean {
  if (status < 200 || status >= 300) return false;
  if (!payload || typeof payload !== "object") return status >= 200 && status < 300;
  const o = payload as Record<string, unknown>;
  if (typeof o.success === "boolean") return o.success;
  if (typeof o.status === "string") {
    const s = o.status.toLowerCase();
    if (s === "error" || s === "fail" || s === "failed") return false;
...
async function fetchOnce(input: ChangeUidInput): Promise<UidApiResult> {
 ...
 */
export async function changeUid(input: ChangeUidInput): Promise<UidApiResult> {
  let last: UidApiResult | undefined;

  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    last = await fetchOnce(input);
    if (last.ok) return last;
    if (!last.retryable || attempt === MAX_RETRIES - 1) return last;
