...
export function addAllowedUser(user) {
    addedUsers.add(user);
    fs.writeFileSync(allowedUsersFile, JSON.stringify(Array.from(addedUsers)), "utf-8");
}
export function isUserAllowed(userId, username) {
    if (env.ALLOWED_ADMIN_IDS.length === 0) {
        return true;
    }
    if (env.ALLOWED_ADMIN_IDS.includes(userId))
        return true;
    if (addedUsers.has(userId))
        return true;
    if (username && addedUsers.has(username.toLowerCase()))
        return true;
    if (username && addedUsers.has(`@${username.toLowerCase()}`))
        return true;
    return false;
}
export async function performChangeUid(userId, raw) {
...
//# sourceMappingURL=changeUidService.js.map